...
export function ProductImage({ src, alt, className = "", eager = false }) {
  ...
  return (
    <div className={`relative aspect-[4/3] overflow-hidden bg-brand-100 ${className}`}>
      ...
      {showImage ? (
        <img src={src} alt={alt} className={`size-full object-cover transition-opacity duration-300 ${state === "loaded" ? "opacity-100" : "opacity-0"}`} loading={eager ? "eager" : "lazy"} decoding="async" onLoad={() => setState("loaded")} onError={() => setState("broken")} />
      ) : (
