...
export function WhatsappOrderCreated({ order }) {
  ...
  return (
    <div id="whatsapp-order-created" tabIndex="-1" className="outline-none focus-visible:ring-2 focus-visible:ring-brand-700" role="status">
      <PageHeader eyebrow="Order recorded" title="Your order is safely with Nuede." description="Continue on WhatsApp to complete the conversation. Your order is unpaid until Nuede verifies payment." />
      <Card as="section" className="mt-8 overflow-hidden p-5 sm:p-8" aria-labelledby="whatsapp-order-reference">
        ...
        <dl className="mt-7 grid gap-4 rounded-control bg-canvas p-5 sm:grid-cols-3">
          <div><dt className="text-xs uppercase tracking-[0.12em] text-muted">Authoritative total</dt><dd className="mt-1 font-display text-2xl text-brand-950">{formatKobo(order.totalKobo)}</dd></div>
          ...
          <div><dt className="text-xs uppercase tracking-[0.12em] text-muted">Fulfilment</dt><dd className="mt-1 font-semibold capitalize text-brand-950">{order.fulfilmentStatus}</dd></div>
        </dl>
        {safeUrl ? (
          <div className="mt-7">
          ...
          <div className="mt-7 rounded-control border border-amber-200 bg-amber-50 p-4" role="alert">
            <p className="font-semibold text-warning">WhatsApp could not be prepared.</p>
            <p className="mt-1 text-sm leading-6 text-warning">Your order still exists. Keep reference <strong>{order.orderReference}</strong> and contact Nuede so the conversation can be recovered.</p>
          </div>
        )}
        <p className="mt-7 flex items-start gap-2 border-t border-line pt-5 text-sm leading-6 text-muted"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-success" aria-hidden="true" />The total above and the WhatsApp message came from the permanent server order, not the earlier checkout estimate.</p>
      </Card>
    </div>
  );
}
