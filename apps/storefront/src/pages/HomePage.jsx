...
export function HomePage() {
  ...
  return <>
    ...
    <section id="faq" className="scroll-mt-24 border-y border-line bg-surface py-16 sm:py-20"><Container className="grid gap-10 lg:grid-cols-[0.7fr_1.3fr]"><div><SectionHeading eyebrow="Good to know" title="Before you order." description="A few answers to help you choose, plan and check out." /><Button to={storefrontPaths.faq} variant="ghost" className="mt-5">View all FAQs</Button></div><div className="divide-y divide-line border-y border-line">{faqs.slice(0, 4).map((item) => <details key={item.question} className="group py-5"><summary className="min-h-11 cursor-pointer py-2 font-semibold text-brand-950">{item.question}</summary><p className="mt-3 max-w-2xl text-sm leading-7 text-muted">{item.answer}</p></details>)}</div></Container></section>
