...
import { MarkWhatsappPaidAction } from "../features/orders/components/MarkWhatsappPaidAction.jsx";
...
export function OrderDetailPage() {
  ...
  return <><AdminPageHeader eyebrow="Order detail" title={order.order_reference} description="Purchase, customer, delivery, nutrition, and payment records." actions={<><Button variant="secondary" to={backTo}><ArrowLeft className="size-4" aria-hidden="true" />Back to orders</Button><DeleteOrderAction order={order} size="medium" onDeleted={() => navigate(backTo, { replace: true })} /></>} /><div className="mt-4 flex flex-wrap items-center gap-2"><OrderStatusBadge type="payment" value={order.payment_status} /><OrderStatusBadge type="fulfilment" value={order.fulfilment_status} /><MarkWhatsappPaidAction order={order} /></div><div className="mt-6"><FulfilmentActions order={order} mutation={mutation} onUpdate={updateStatus} /></div><div className="mt-5 grid gap-5 xl:grid-cols-2"><OrderIdentityCard order={order} /><OrderCustomerCard order={order} /><OrderDeliveryCard order={order} /><OrderTotalsCard order={order} /></div><div className="mt-5 grid gap-5"><MealPlanScheduleCard order={order} /><OrderItemsCard items={order.order_items} /><OrderPaymentCard order={order} /></div></>;
}
