...
export function SettingsPage() {
  ...
  return (
    <><AdminPageHeader eyebrow="Configuration" title="Settings" description="Control which payment methods customers can use at checkout." />
      <div className="mt-6 max-w-3xl">
        ...
        {query.isSuccess ? <Panel className="p-5 sm:p-7"><div className="flex items-start gap-3"><div className="grid size-10 shrink-0 place-items-center rounded-full bg-brand-100 text-brand-700"><ShieldCheck className="size-5" aria-hidden="true" /></div><div><h2 className="text-xl font-semibold text-brand-950">Customer checkout methods</h2><p className="mt-1 text-sm leading-6 text-muted">Disabled methods disappear from checkout after its next refresh. The database rejects attempts to disable both.</p></div></div>{!canManagePayments ? <p className="mt-4 text-sm text-muted" role="status">Only owners and administrators can change payment methods.</p> : null}<form className="mt-6 grid gap-5" noValidate onSubmit={handleSubmit(save)}><CheckboxField disabled={!canManagePayments || mutation.isPending} label="Paystack available" help="Allows new orders to pay through Paystack hosted checkout." {...register("paystackEnabled")} /><CheckboxField disabled={!canManagePayments || mutation.isPending} label="WhatsApp available" help="Records the order before opening the WhatsApp handoff." {...register("whatsappEnabled")} />{!values.paystackEnabled && !values.whatsappEnabled ? <p className="rounded-control bg-amber-50 p-3 text-sm text-warning" role="alert">Enable at least one method before saving.</p> : null}{errors.root?.message ? <p className="text-sm text-danger" role="alert">{errors.root.message}</p> : null}<Button type="submit" busy={mutation.isPending} disabled={!canManagePayments || (!values.paystackEnabled && !values.whatsappEnabled)}>Save checkout settings</Button></form></Panel> : null}
      </div>
    </>
  );
}
